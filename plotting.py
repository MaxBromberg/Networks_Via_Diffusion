...
G = graph.EffDisGraph(num_nodes, edge_weighting_exp_rate=5, rate_of_edge_adaptation=6)
# G.sparse_random_edge_init(outgoing_edges_per_node)
G.uniform_random_edge_init()
G.run(num_runs, exp_decay_param=0.1, constant_source_node=1, multiple_path=False, verbose=True)
...
print(f'Initial x values: {np.round([(G.edge_weighting_exp * (G.nodes[1][i] / G.nodes[1].sum())) for i in range(G.nodes.shape[1])], 3)}')
print(f'Final x values: {np.round([(G.edge_weighting_exp * (G.nodes[-1][i] / G.nodes[-1].sum())) for i in range(G.nodes.shape[-1])], 3)}')
