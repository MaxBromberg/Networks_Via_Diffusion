...
G = graph.Graph(num_nodes=num_nodes, edge_conservation_coefficient=edge_conservation,
                selectivity=edge_selectivity, reinforcement_info_score_coupling=True)
# G.uniform_random_edge_init()
# G.barabasi_albert_edge_init(num_edges_per_new_node=int(num_nodes / 10))
G.sparse_random_edge_init(3, connected=True, even_distribution=True)
print(G.A[-1])
utility_funcs.print_row_column_sums(G.A[-1])
utility_funcs.sum_matrix_signs(G.A[-1])
G.simulate(num_runs, eff_dist_delta_param=2, constant_source_node=False, num_shifts_of_source_node=False, seeding_power_law_exponent=15, seeding_sigma_coeff=False, beta=None, equilibrium_distance=200, multiple_path=False, verbose=True)
print(G.A[-1])
utility_funcs.print_row_column_sums(G.A[-1])
utility_funcs.sum_matrix_signs(G.A[-1])
