...
class EffectiveDistances:
    ...
    def load(self, fname, verbose=True, **kwargs):
        ...
        if set(range(self.nodes)) != nodes:
            ...
            print
            'Lines: ', lines, ', Nodes: ', self.nodes
            print
            '-----------------------------------\nData Structure:\n\nsource,    target,    weight \n'
            for ii in range(7):
                print
                "%i,       %i,       %1.2e" % (source[ii], target[ii], flux[ii])
            print
            '-----------------------------------\n'
        ...
        if verbose:
            if not symmetric:
                print
                "The network has been symmetricised."
...
if __name__ == "__main__":
    ...
    # distances = G.get_multiple_path_distance(source=0, target=None, parameter=3.,
    #                                          saveto="/home/andreasko/effective_distance/line_graph_MPED")
    # print
    # "Multiple Path Effective Distance :", distances
