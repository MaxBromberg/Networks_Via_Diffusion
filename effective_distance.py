"""
Original version by Dr. Andreas Koher found at https://github.com/andreaskoher/effective_distance. Thanks Andreas!

Code modified to allow for data loading directly from numpy array for compatibility with simulation data format output
To update for NetworkX 2.2+, I also shifted strongly_connected_component_subgraphs(G) to (G.subgraph(c).copy() for c in strongly_connected_components(G)
as recommended here: https://github.com/networkx/networkx/pull/2819/commits/b5a323230e2a9e324a8e455086b89c85ab366c08
...
from networkx import DiGraph, shortest_path_length, adjacency_matrix, shortest_path, all_simple_paths, \
    strongly_connected_components, convert_matrix, from_numpy_matrix
...
class EffectiveDistances:
    def __init__(self, fname="", np_array=None, verbose=True, **kwargs):
        assert isinstance(fname, str) or np_array is not None
        self.graph = None
        self.nodes = 0
        ...
        if np_array.any():
            self.graph = self.load_from_numpy(np_array)
    ...
    def get_dominant_path_distance(self, source=None, target=None, parameter=1, saveto=""):
        ...
        if source or target:
            ...
            DPED_dic = dict(shortest_path_length(self.graph, source=source, target=target, weight="effective_distance"))

        if source is None and target is None:
            DPED = np.array([DPED_dic[s].values() for s in list(range(self.nodes))]).transpose()
            # DPED = np.array([DPED_dic[s].values() for s in xrange(self.nodes)]).transpose() EDITED: to be Python 3.7 compatible
