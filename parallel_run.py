...
output_directory, run_index, num_nodes, coupling_val, adaptation_val
...
if __name__ == '__main__':
    G = graph.EffDisGraph(num_nodes=num_nodes, eff_dist_and_edge_response=coupling_val, rate_of_edge_adaptation=adaptation_val)
    G.uniform_random_edge_init()
    G.run(num_runs, exp_decay_param=1, constant_source_node=1, equilibrium_distance=200, multiple_path=False)
    plotter.save_object(G, Path(output_path, f'{run_index:04}_graph_obj.pkl'))
    print(f'Run {run_index}, [coupling_val: {coupling_val}, edge_adaptation: {adaptation_val}] complete. \n Raw graph data recorded at: {output_path}')
